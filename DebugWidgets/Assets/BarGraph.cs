...
public class BarGraph : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        if (Min < 0 || Max < 0) {
            var currPosition = BarTransform.localPosition;
            currPosition.x = width / 2;
            BarTransform.localPosition = currPosition;
            signedDisplay = true;
        }
    }
    ...
    public void SetReading(float value)
    {
        ...
        Text.text = this.gameObject.name + " : " + passedValue;
    }
    ...
    public void SetWidthPercent(float value, Color c)
    {
        ...
        var currentScale = BarTransform.localScale;
        currentScale.x = value * currentScale.y;
        if (signedDisplay) currentScale.x = currentScale.x * 0.5f;
